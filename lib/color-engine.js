...
/**
 * 图文蒸馏新增（63图实证 C3/C5）：生成单色相阶梯（Single-Hue Ladder）
 *
 * 原则 C3 单色相深耕：一张图只讲一个色相家族，Δh < 15°，跨色相跳出色 ≤5%。
 * 原则 C5 冷暖呼吸：明度阶梯不做线性渐变，在中段插入 1~2 个冷暖偏移形成呼吸节奏。
 *
 * @param {Object} params
 * @param {number} params.hue - 基准色相 0-360
 * @param {number} [params.steps=7] - 阶梯数（推荐 5~7）
 * @param {number} [params.saturation=45] - 饱和度 0-100
 * @param {number} [params.lMin=18] - 最深明度
 * @param {number} [params.lMax=82] - 最浅明度
 * @param {number} [params.hueDrift=8] - 允许的冷暖色相漂移（°，对应 Δh < 15° 的一半）
 * @param {string[]} [params.names] - 诗意名（可选，按步骤顺序）
 * @returns {{hue:number, steps:Array<{hex:string,lightness:number,hue:number,name?:string,breath:string}>}}
 */
export function generateSingleHueLadder(params = {}) {
  const {
    hue = 280,
    steps = 7,
    saturation = 45,
  ...
  for (let i = 0; i < steps; i++) {
    ...
    const lightness = lMax - (lMax - lMin) * t;

    // 冷暖呼吸：奇数段（中段）做 ±hueDrift 的色相偏移，形成波浪
    // 节奏示例：冷-冷-暖-暖-金-冷-暖（呼吸点在中段）
    let hueOffset = 0;
    let breath = 'neutral';
    if (steps >= 5) {
      const mid = Math.floor(steps / 2);
      if (i < mid - 1) { hueOffset = -hueDrift; breath = 'cool'; }
      else if (i === mid - 1) { hueOffset = 0; breath = 'cool'; }
      else if (i === mid) { hueOffset = hueDrift * 0.6; breath = 'warm'; }
      else if (i === mid + 1) { hueOffset = hueDrift; breath = 'gold'; }
      else { hueOffset = -hueDrift * 0.5; breath = 'warm'; }
    } else {
      hueOffset = (i % 2 === 0 ? -hueDrift : hueDrift);
      breath = i % 2 === 0 ? 'cool' : 'warm';
    }
  ...
  return { hue, steps: ladder, accent: '#E8C870' };
}

/** HSL → HEX（引擎内部小工具，避免依赖 utils） */
...
/**
 * 图文蒸馏新增（63图实证 C1）：校验冷场暖点配色
 * @param {string[]} colors - 全部配色 hex
 * @param {Object} meta - { warmAreaPct, redPointHex, redPointPct }
 * @returns {{pass:boolean, violations:string[]}}
 */
export function validateCoolFieldWarmPoint(colors = [], meta = {}) {
  const violations = [];
  const { warmAreaPct = 0, redPointHex = '', redPointPct = 0 } = meta;

  if (warmAreaPct > 5) violations.push(`暖色面积 ${warmAreaPct}% > 5%，冷场被稀释`);
  if (redPointHex && redPointPct > 2) violations.push(`朱红单点面积 ${redPointPct}% > 2%，焦点变成红色主体`);
  if (colors.length > 3) violations.push(`配色数 ${colors.length} > 3（冷场+金+一点朱红），暖色过多`);
